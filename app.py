...
def cargar_datos_eficacia(modo_demo):
    if modo_demo:
        ...
        try:
            ...
            # CSO (Corrección Silenciosa del Typo dm_Efiacia)
            try: df_e_cso = conn.query("SELECT * FROM dm.Eficacia_CSO", ttl=3600)
            except: df_e_cso = conn.query("SELECT * FROM dm_Efiacia_CSO", ttl=3600) # Fallback typo original
            
            df_e_cso['Fecha'] = pd.to_datetime(df_e_cso['Fecha'])
            return df_e_glob, df_e_cso
        except Exception as e:
            st.error(f"⚠️ Error SQL Eficacia: {e}")
...
with st.sidebar:
    ...
    # AGREGADAS LAS PAGINAS NUEVAS AL MENU
    pagina = st.radio("MENÚ PRINCIPAL", ["📊 Telemetría en Vivo", "🚦 Telemetría de Tráfico (PVS)", "🏢 Modelo Eficacia Total", "🔮 Estrategia & Predicción", "🧠 Chat Gari IA"])
...
if pagina == "📊 Telemetría en Vivo":
    ...
    # 2. CRUCE DE DATOS CON DF_RAW PARA OBTENER ZONAS
    if 'Sucursal_Upper' not in df_raw.columns: df_raw['Sucursal_Upper'] = df_raw['Sucursal'].astype(str).str.upper().str.strip()
    df_eff_cso['Sucursal_Norm'] = df_eff_cso['Sucursal'].astype(str).str.strip().str.upper()
    
    try: df_mapping = df_raw[['Sucursal_Upper', 'ZONA', 'RED']].drop_duplicates(subset=['Sucursal_Upper'])
